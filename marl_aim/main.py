...
from utils import compute_edges, compute_rp, compute_connections
from memory import ReplayBuffer

# Runs policy for X episodes and returns average reward
# A fixed seed is used for the eval environment
def eval_policy(policy, params, eval_episodes=10):
	...
	while not done:
            ...
            for i, ids in enumerate(list(state.keys())):
                ...
                if num_conn[i] == -1:
                    ...
                    action = policy[num_conn[i]].select_action(np.array(aug_state))
                actions_for_env.append(action)

		state, reward_list, done, _ = env.step(action)
...
if __name__ == "__main__":
	...
	vehicles = VehicleParams()
        ...
        flow_params_eval['env'].horizon = 1000
	...
	if args.policy == "TD3":
		...
		kwargs["policy_freq"] = args.policy_freq
                for i in range(4):
                    ...
                    policy.append(TD3.TD3(state_dim[i], **kwargs))
		...
		episode_timesteps += 1
                ...
                for i, ids in enumerate(list(state.keys())):
                    ...
                    if num_conn[i] == -1:
                        ...
                        action = (policy[num_conn[i]].select_action(np.array(aug_state)) + np.random.normal(0, max_action*args.expl_noise, size=action_dim)).clip(-max_action, max_action)
                    if num_conn[i] != -1:
                        aug_states[num_conn[i]].append(aug_state)
                        actions[num_conn[i]].append(action)
                    actions_for_env.append(action)

                state_, reward_list, done, _ = env.step(actions_for_env)
                ...
                for i, ids in enumerate(list(state.keys())):
                    if ids in list(state_.keys()):
                        ...
                        num_conn_.append((len(aug_state)-3)/4 - 1)
                        if num_conn[i] != -1:
                            if num_conn_[i] > num_conn[i]:
                                ...
                                for j in diff:
                                    aug_state.append(0)
                            aug_states_[num_conn[i]].append(aug_state)
                            rewards[num_conn[i]].append(reward_list[i])
                    else:
                        aug_state = list(np.zeros(num_conn[i]))
                        aug_states_[num_conn[i]].append(aug_state)
                        rewards[num_conn[i]].append(reward_list[i])
