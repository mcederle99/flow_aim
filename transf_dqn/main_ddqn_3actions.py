...
import numpy as np
import torch
from utils_ddqn_3actions import flow_params, trim, order_vehicles, map_actions
from memory import ReplayBuffer
from agent_ddqn_3actions import DQN
...
for i in range(num_eps):
    ...
    for j in range(max_ep_steps):    
        ...
        next_state, reward, done, crash = env.step(env_actions)
    ...
    np.save('results/ddqn/returns.npy', returns_list)
    np.save('results/ddqn/ep_steps.npy', ep_steps_list)
    np.save('results/ddqn/returns_per_veh.npy', returns_per_veh_list)
    aim.save()
