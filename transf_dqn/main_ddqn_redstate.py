...
import numpy as np
import torch
from utils_ddqn_redstate import flow_params, trim, order_vehicles, map_actions
from memory_redstate import ReplayBuffer
from agent_ddqn_redstate import DQN
...
memory = ReplayBuffer(12, 1)

aim = DQN(12, 1, eps_dec=1e-6, filename='models/ddqn_redstate/aim')
...
for i in range(num_eps):
    ...
    np.save('results/ddqn_redstate/returns.npy', returns_list)
    np.save('results/ddqn_redstate/ep_steps.npy', ep_steps_list)
    np.save('results/ddqn_redstate/returns_per_veh.npy', returns_per_veh_list)
    aim.save()
