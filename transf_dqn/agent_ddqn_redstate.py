...
from network_redstate import Qfunc
...
class DQN():
    ...
    def select_action(self, state):
        num_vehs = state.shape[0] // 12 
        if np.random.random() > self.epsilon:
            state = state.view(1, -1, 12).to(device)
            actions = self.q(state)
            action = torch.argmax(actions, -1).squeeze()
        else:
            action = torch.randint(0, 21, (num_vehs,), dtype=torch.long)
    ...
    def train(self, replay_buffer, batch_size=batch_size):
        ...
        state = state.view(batch_size, -1, 12)
        next_state = state.view(batch_size, -1, 12)
        action = F.one_hot(action.to(torch.long), num_classes=21)
        ...
        with torch.no_grad():
            next_act = self.q(next_state).max(dim=2)[1]
            next_act = F.one_hot(next_act, num_classes=21)
            q_next = self.q_target(next_state)
            q_next = (q_next * next_act).sum(-1)
