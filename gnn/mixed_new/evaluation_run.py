...
from utils_fair import compute_pareto_front
import argparse
import warnings
import pickle
...
for i in range(100):
    ...
    #
    # time_fuel, time_elec = [], []
    # for name_fuel, name_elec in zip(list(dictionary_name_time_fuel.keys()), list(dictionary_name_time_elec.keys())):
    #     time_fuel.append(dictionary_name_time_fuel[name_fuel])
    #     time_elec.append(dictionary_name_time_elec[name_elec])
    #
...
# sorted_fairness = sorted(delta_fairness, key=lambda x: x)
#
...
# plt.close()
#
# plt.scatter(np.arange(len(delta_fairness)), sorted_fairness)
# plt.show()
# plt.close()
...
with open(f"delta_fairness_{args.nn_architecture}_{args.seed}.pkl", "wb") as f:
