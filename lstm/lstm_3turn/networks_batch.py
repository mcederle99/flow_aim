...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        for j in range(num_vehs): # for every vehicle present in that instant
            ...
            hx, cx = self.lstm(current_state, (hx, cx)) # iterate in the LSTM cell
        hx = torch.cat((state[:, 0:15], hx), dim=1) # concatenate the ego_state with the final LSTM output
...
class Critic(nn.Module):
    ...
    def forward(self, state, action):
        ...
        hx_1 = torch.zeros((batch_size, 256), device=self.device, dtype=torch.float32)
        cx_1 = torch.zeros((batch_size, 256), device=self.device, dtype=torch.float32)
        hx_2 = torch.zeros((batch_size, 256), device=self.device, dtype=torch.float32)
        cx_2 = torch.zeros((batch_size, 256), device=self.device, dtype=torch.float32)
        for j in range(num_vehs): # for every vehicle present
            current_state = state[:, 15*j:15*(j+1)]
            hx_1, cx_1 = self.lstm_1(current_state, (hx_1, cx_1)) # iterate in the LSTM cell
            hx_2, cx_2 = self.lstm_2(current_state, (hx_2, cx_2)) # iterate in the LSTM cell
        
        hx_1 = torch.cat((state[:, 0:15], action.unsqueeze(dim=1), hx_1), dim=1) # concatenate the ego_state with the final LSTM output
        hx_2 = torch.cat((state[:, 0:15], action.unsqueeze(dim=1), hx_2), dim=1) # concatenate the ego_state with the final LSTM output
