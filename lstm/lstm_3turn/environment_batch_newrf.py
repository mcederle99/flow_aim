...
from utils_batch_newrf import order_vehicles
...
#            dones = torch.cat((dones, done))
    ...
    def get_state(self):
        ...
        state = torch.zeros((len(ord_vehs), 15*len(ord_vehs)))
        ...
        if num_arrived > 0:
            if len(ids) > 0:
                aug_col = torch.zeros((len(ids), 15*num_arrived))
                aug_row = torch.zeros((num_arrived, 15*(state.shape[0]+num_arrived)))
                state = torch.cat((state, aug_col), dim=1)
                state = torch.cat((state, aug_row), dim=0)
            else:
                state = torch.zeros((num_arrived, 15*num_arrived))
