...
class SpeedEnv(Env):
    ...
    * sort_vehicles: specifies whether vehicles are to be sorted by position
    ...
    def observation_space(self):
        ...
        obs_space = Box(low=-1, high=1, shape=(vehs,self.state_dim))
...
#                reward += torch.tensor([displ - 1.0])
                 vel = self.k.vehicle.get_speed(i)
                 reward += vel/15.5
                 acc = self.k.vehicle.get_realized_accel(i)
                 if acc < -1.0:
                     reward -= 0.5
            else:
                reward += torch.tensor([2.0])
    ...
    def get_state(self):
        ...
        for q in ids:
            ...
            vel = self.k.vehicle.get_speed(q)/15.5
            if np.abs(vel) > 50:
                vel = 0.0
            obs.append(vel)
